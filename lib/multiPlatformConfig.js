...
function checkEnvironmentVariables() {
  ...
  const foundEnvVars = envVarsToCheck.filter(v => process.env[v]);

  if (foundEnvVars.length > 0) {
    console.log(chalk.yellow('\n⚠️  检测到以下环境变量，可能会覆盖配置文件的设置：'));
    foundEnvVars.forEach(v => {
      console.log(chalk.yellow(`   - ${v}`));
    });
    console.log(chalk.yellow('请清除这些环境变量以确保配置切换生效'));
    console.log(chalk.gray('提示: 可以使用 unset ANTHROPIC_BASE_URL ANTHROPIC_AUTH_TOKEN 清除\n'));
  }
}
